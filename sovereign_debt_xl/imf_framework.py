...
@xl_func(
    "float[] gdp_path, float[] fiscal_path, float[] interest_path,"
    " float[] exchange_rate_path, float[] financing_assumptions: object[][]",
    name="SOV_DSA_REPLICATION",
)
def dsa_replication(
    gdp_path: Any,
    fiscal_path: Any,
...
@xl_func(
    "float reserves_months_imports, float debt_gdp, float current_account_gdp,"
    " float inflation, float exchange_rate_regime, float political_stability: float",
    name="SOV_IMF_PROGRAM_PROBABILITY",
)
...
@xl_func(
    "float debt_gdp, float gross_financing_need_gdp, float market_access_boolean,"
    " str debt_sustainability_assessment: object[][]",
    name="SOV_EXCEPTIONAL_ACCESS_CHECK",
)
...
@xl_func(
    "float sdr_allocation, float gdp, float reserves, float import_cover_pre: object[][]",
    name="SOV_SDRS_ALLOCATION_IMPACT",
)
