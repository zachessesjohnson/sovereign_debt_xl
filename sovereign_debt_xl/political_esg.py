...
@xl_func(
    "float polity_iv_score, float[] wgi_governance_indicators, float years_since_last_default,"
    " float regime_change_dummy, float election_proximity_months: object[][]",
    name="SOV_POLITICAL_RISK_SCORE",
)
...
@xl_func(
    "float co2_per_capita, float renewable_energy_share, float gini_coefficient,"
    " float rule_of_law_index, float education_spending_gdp, float health_spending_gdp: object[][]",
    name="SOV_ESG_SOVEREIGN_SCORE",
)
...
@xl_func(
    "float[] trade_partner_shares, float[] fx_reserves_by_currency_share,"
    " float swift_dependency_pct, float energy_export_share: object[][]",
    name="SOV_SANCTIONS_EXPOSURE",
)
