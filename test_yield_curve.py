...
from sovereign_debt_xl.yield_curve import asm_spread, carry_rolldown, nelson_siegel_fit, zspread
...
def test_asm_spread_par_near_zero():
    ...
    asw = asm_spread(par_price, 4.0, 4.0, tenors, rates)
    assert abs(asw) < 1e-5
...
def test_asm_spread_premium_bond_negative():
    ...
    asw = asm_spread(105.0, 4.0, 5.0, tenors, rates)
    assert asw < 0
